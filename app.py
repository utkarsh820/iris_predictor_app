        ...
        try:
            ...
            if prediction and 'Error' not in prediction:
                ...
                bars = ax.bar(['Sepal Length', 'Sepal Width', 'Petal Length', 'Petal Width'],
                        [sl, sw, pl, pw],
                        color=['#00f3ff', '#0fff50', '#ff00ff', '#ff355e'])

                # Customize plot
                ax.set_title('Iris Measurements', color='white', pad=20, fontsize=14)
                ax.set_ylabel('Centimeters (cm)', color='white')
...
<head>
    ...
    <style>
        ...
        iframe { width: 300px; height: 200px; margin-top: 10px; border: none; }
    </style>
</head>
<body>
    ...
    <form method="post">
        ...
        <input type="number" step="0.1" name="sl" required><br>
        <label for="sw">Sepal Width (cm):</label>
        <input type="number" step="0.1" name="sw" required><br>
        <label for="pl">Petal Length (cm):</label>
        <input type="number" step="0.1" name="pl" required><br>
        <label for="pw">Petal Width (cm):</label>
        <input type="number" step="0.1" name="pw" required><br>
